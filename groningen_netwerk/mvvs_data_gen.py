...
def get_mvvs_data(ndw_links, debug=False):
    ...
    ax.set_aspect('equal')
    ax.imshow(im, extent=(0, 1115 * 1000 / 108 / 0.992, 0, 964 * 1000 / 108 / 0.992))  # 1 pixel ~ 1x1 meter
    g = nx.DiGraph()
...
if __name__ == "__main__":
    ...
    handle = dt.Speed_MVVS_handler('STNN_master/data/groningen_mvvs_data_links.csv', min_count_fraction=.98)
    with open('STNN_master/data/groningen_mvvs/labels.pkl', 'rb') as f:
        ndw_subset = pickle.load(f)
    handle_i = dt.MVVS_handler('STNN_master/data/groningen_mvvs_data_links.csv',
                               loc_list=[str(x) for x in ndw_subset], min_count_fraction=.98)
    i_dfs = pd.concat(handle_i.dfs)
    ndw_locs = [int(x) for x in handle_i.locations]
    ...
    nx.draw_networkx_nodes(g, linkpos, nodelist=[int(ndw_locs[i]) for i in fnodes5], node_color='b', node_size=100, ax=ax)
    ...
    def speed_graph(row):
        ...
        #         except nx.exception.NetworkXNoPath:
        #             s_adjmat[i, j] = np.inf
        s_adjmat = nx.floyd_warshall_numpy(ndw_tg, weight='weight')
        return s_adjmat

    if not rev or not static:
        xmats_list = []
    ...
    def loc_to_arr(df, loc, i=None):
        ...
        # densities = intensities/(speeds+1)  # v/km = v/h / km/h
        return np.stack([speeds, intensities]).T

    subdfs = [i_dfs.loc[df.index] for df in handle.dfs]
    datasets = [np.stack([loc_to_arr(df, str(loc), i) for loc in ndw_subset]) for i, df in tqdm(enumerate(subdfs)) if len(df)>34]
    with open(f'STNN_master/data_na_selfless/{rev_pre}groningen_mvvs_sl_test/adjmat_labels.pkl', 'wb') as f:
        pickle.dump(labels, f)
    with open(f'STNN_master/data_na_selfless/{rev_pre}groningen_mvvs_sl_test/nodeval_labels.pkl', 'wb') as f:
        pickle.dump(ndw_subset, f)

    # assert len(xmats_list) == len(datasets)

    for i in range(len(datasets)):
        os.mkdir(f'STNN_master/data_na_selfless/{rev_pre}groningen_mvvs_flow_sl_test/part{i}')
        with open(f'STNN_master/data_na_selfless/{rev_pre}groningen_mvvs_flow_sl_test/part{i}/node_values.npy', 'wb') as f:
            np.save(f, datasets[i])
        with open(f'STNN_master/data_na_selfless/{rev_pre}groningen_mvvs_flow_sl_test/part{i}/adj_mat.npy', 'wb') as f:
            if rev or static:
                np.save(f, adj_mat)
            else:
                np.save(f, xmats_list[i])
        with open(f'STNN_master/data_na_selfless/{rev_pre}groningen_mvvs_flow_sl_test/part{i}/physmat.npy', 'wb') as f:
            np.save(f, p_mat)
